...
def askQuestions(Query):
    """Send a natural language question to the AI and print the response."""
    try:
        client = Groq(api_key=groq_api_key)
        response = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "You are a CLI expert. Provide concise, actionable answers."},
        ...
        print(f"Error occurred: {e}. Please check the network connection")
        return

def executeQuery(Query):
    """Send a natural language query to the AI and return a list of shell commands to execute."""
    try:
        client = Groq(api_key=groq_api_key)
        content = (
            "You are a command line expert. Convert the following natural language request "
            "into a Python list of shell commands for Windows Command Prompt. Output ONLY the list in format ['cmd1', 'cmd2']. No explanation. "
            f"Request: {Query}"
        )
        ...
        response = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "You are a command line expert that generates Windows commands as Python lists."},
                {"role": "user", "content": content}
            ]
        )

        response_text = response.choices[0].message.content.strip()
        # Try code blocks first; fall back to parsing the raw response text directly
        code_blocks = re.findall(r"```(?:\w+)?\s*([\s\S]+?)```", response_text)
        candidates = [b.strip() for b in code_blocks] + [response_text]
        for candidate in candidates:
            try:
                ...
                continue
        print("Error occurred. Please check the network connection, we are Trying Again")
        return []
    except Exception as e:
        print(f"Network error: {e}. Please check the network connection, we are Trying Again")
        return []

def suggest_commands(cmd_history):
    """Ask the AI for the next 5 command suggestions based on recent command history."""
    try:
        client = Groq(api_key=groq_api_key)
        response = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "You are a CLI expert. Suggest next 5 commands as a Python list only."},
                {"role": "user", "content": f"Command history: {cmd_history}\nSuggest next 5 commands as a list: ['cmd1', 'cmd2', ...]"}
            ]
        )
        response_text = response.choices[0].message.content.strip()
        # Try code blocks first; fall back to parsing the raw response text directly
...
class PathCompleter(Completer):
    def get_completions(self, document, complete_event):
        """Yield path completions for 'cd' and 'activate' commands."""
        ...
        try:
            ...
            partial_name = os.path.basename(partial).lower() if partial else ""
...
class CompositeCompleter(Completer):
    """Route completions to PathCompleter for path commands, WordCompleter for everything else."""
    ...
    def get_completions(self, document, complete_event):
        """Delegate to the appropriate completer based on the leading command word."""
        text = document.text_before_cursor.strip()
        words = text.split()
...
class AutoSuggestCmd(AutoSuggest):
    def get_suggestion(self, buffer, document):
        """Return an inline ghost-text suggestion matching the current input prefix."""
        text = document.text.strip()
        if not text:  # strip() always returns str, never None — use falsy check
            return None
...
def command_prediction_async():
    """Fetch AI-predicted commands in a background thread and add them to the completer."""
    with lock:
...
@bindings.add('c-t')
def _(event):
    """Ctrl+T key binding: trigger AI command prediction in the background."""
    threading.Thread(target=command_prediction_async, daemon=True).start()

def run_in_docker(cmd, timeout=5):
    """Run a shell command inside a sandboxed Docker container and return its output."""
    # Check if Docker is available
    try:
        subprocess.run(["docker", "--version"], capture_output=True, timeout=2)
    except (FileNotFoundError, subprocess.TimeoutExpired):
        return "[!] Docker is not installed or not running. Skipping sandbox execution."
...
while True:
    try:
        ...
        venv_prefix = f"({os.path.basename(os.environ.get('VIRTUAL_ENV', ''))}) " if os.environ.get('VIRTUAL_ENV') else ""
        text = prompt(
        ...
        if text.lower() == 'exit':
            ...
            try:
                # Check whether the command matches any dangerous pattern before running
